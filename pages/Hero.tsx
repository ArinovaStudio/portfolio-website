...
// ✅ Lazy import to prevent WebGL compile during text animation
const ThreeD = React.lazy(() => import("@/elements/Canvas"));

function Hero() {
  const [isMobile, setIsMobile] = useState(false);
  const [showModel, setShowModel] = useState(false); // ⏳ control model mount
  ...
  useEffect(() => {
    ...
    // 🎬 wait for GSAP animation to complete (~3.8s)
  ...
  return (
    <AuroraBackground>
      <div className="w-screen h-screen text-white flex items-center justify-center flex-col relative overflow-hidden">
        {/* ✅ 3D Model (loads *after* text animation completes) */}
        {showModel && (
            <motion.div
              className="w-screen h-screen absolute z-20 flex justify-center items-center flex-col"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
  ...
  <motion.h1
    initial={{ opacity: 0 }}
    animate={{ opacity: 1 }}
    transition={{ delay: 1, duration: 0.9 }} // 0.4s after showModel becomes true
